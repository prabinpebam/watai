...
export function buildDockerValidationArgs(
  ...
  return [
    ...
    "--tmpfs", `/workspace/run:rw,nosuid,mode=1777,size=${manifest.runtime.memoryMb}m`,
    ...
    "/bin/sh",
    "-c",
    "workdir=\"$1\"; dependency_root=\"$2\"; shift 2; link_dependencies() { mkdir \"$2\" && find \"$1\" -mindepth 1 -maxdepth 1 -exec ln -s '{}' \"$2/\" ';'; }; cp -R /workspace/source/. /workspace/run/ && test -d \"$dependency_root\" && if [ ! -e /workspace/run/node_modules ]; then link_dependencies \"$dependency_root\" /workspace/run/node_modules; fi && if [ -d \"$dependency_root/../api/node_modules\" ] && [ -d /workspace/run/api ] && [ ! -e /workspace/run/api/node_modules ]; then link_dependencies \"$dependency_root/../api/node_modules\" /workspace/run/api/node_modules; fi && cd \"$workdir\" && exec \"$@\"",
    "watai-validation",
    workdir,
