...
export function buildDoctorReport(input: DoctorInput): HarnessDoctorReport {
  ...
  const hSlices: DoDControl[] = [
    ...
    control(
      ...
      "Independently attest the local execution components, supply an approved worker image, and replace same-host SQLite with distributed CAS before release authority.",
    ),
    control(
      "H04",
      ...
      "Move the passing fixed evaluator into independently signed isolated build infrastructure and immutable evidence storage.",
    ),
    control(
      "H05",
