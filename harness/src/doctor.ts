...
export function buildDoctorReport(input: DoctorInput): HarnessDoctorReport {
  ...
  const hSlices: DoDControl[] = [
    ...
    control(
      ...
      [
        ...
        "budget/effect reducers",
        "Copilot SDK empty-mode worker contract",
        "local rehearsal ledger",
      ],
      codes(input.implementation),
      "Supply agent-provider auth, durable CAS/outbox, budget reservation, worktree, structured tool gateway, credential broker, watchdog, effect reconciler and Docker isolation attestations.",
    ),
    control(
      "H04",
      input.evaluation.ready ? "READY_FOR_EVIDENCE" : "BLOCKED",
      ["candidate evidence admission", "exact gate/acceptance/negative-control checks"],
      codes(input.evaluation),
      "Freeze and independently attest the evaluator pack, trusted build and complete browser inventory; run three complete clean browser matrices.",
    ),
    control(
      "H05",
  ...
  const gates: DoDControl[] = [
    ...
    control("G05", input.implementation.ready ? "IMPLEMENTED_LOCAL" : "BLOCKED", ["replay/fencing/budget/effect reducers"], codes(input.implementation), "Run 100 fault schedules and 1,000 seeded schedules on durable adapters."),
    control("G06", "NOT_STARTED", [], ["MEMORY_EVALUATOR_NOT_FROZEN"], "Complete E01 and shipped-pipeline paired memory evaluation."),
    control("G07", input.evaluation.ready ? "IMPLEMENTED_LOCAL" : "BLOCKED", ["evidence contract only"], codes(input.evaluation), "Run critical browser/accessibility/device matrix and record unsupported cells."),
    control("G08", input.implementation.ready ? "IMPLEMENTED_LOCAL" : "BLOCKED", ["quota reservation reducer", "zero-network tool contract"], codes(input.implementation), "Attest concurrent durable reservations and content-safe telemetry."),
