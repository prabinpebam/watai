> **⚠ ARCHIVED (v1 — client calls Azure OpenAI directly).** This document specified how
> the **browser** talked to Azure OpenAI with the user's local key. As of the 2026
> direction change, the **server** owns all Azure OpenAI calls (the browser never holds a
> key). Kept for historical reference only. **See the current specs:**
> [03-api-integration.md](../03-api-integration.md) (server-side call path) and
