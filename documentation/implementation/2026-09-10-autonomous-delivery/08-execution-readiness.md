...
**Current machine result:** `BLOCKED_SAFE`.
**Next canonical slice:** H01.
**Safe today:** plan validation, local preflight, deterministic workflow rehearsal,
cryptographic verification tests, scheduler tests, worker-policy tests and evidence
admission tests.
**Not safe today:** unattended model dispatch, candidate mutation, independent
acceptance, cloud staging, deployment or release.

Dependency audit is also open: root reports 1 critical/4 high/6 moderate and API
reports 1 critical/4 high/3 moderate advisories. The direct Vitest/Vite remedies
currently require major toolchain changes, including a prerelease Vitest target;
do not auto-upgrade or waive these. The future trusted-build attestation must bind
an independently reviewed mitigation or compatible approved upgrade.
...
- Resolve current local blockers: Docker Desktop server is stopped; deployed
  source/runtime/config are unobserved. Azure CLI and Functions tools are locally
  present; no cloud query was performed in this candidate.
