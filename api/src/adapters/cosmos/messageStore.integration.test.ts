...
describe('CosmosMessageStore (integration)', () => {
  ...
  afterAll(async () => {
    ...
    for (const { threadId, id } of created) {
      await c.item(ownerScopedDocumentId('message', 'owner', id), threadId).delete().catch(() => undefined);
    }
  });
  ...
  it('append + get round-trips within the thread partition', async () => {
    ...
    const got = await store.get('owner', threadA, 'm1');
    expect(got?.content).toBe('m1');
  });

  it('cross-thread get returns null (partition isolation)', async () => {
    expect(await store.get('owner', threadB, 'm1')).toBeNull();
  });
  ...
  it('list returns non-deleted oldest-first; since + limit narrow the window', async () => {
    ...
    const ids = (await store.list('owner', threadA)).map((m) => m.id);
