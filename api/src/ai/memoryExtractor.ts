...
export async function extractMemories(
  ...
  const system = [
    ...
    'Store durable facts, preferences, instructions, work style, avoidances, or procedures that will be useful in future conversations.',
    'NEVER store project or work-in-progress context — what is being built or worked on in this thread, task specifics, one-off request parameters, or deployment / repository / project details, or "completed-work" context. Users open separate threads to isolate different approaches, and this context must NOT cross-bleed between threads. Return ignore for it.',
    'The input messages are the conversation in chronological order. Extract durable facts from the LATEST turn (the final messages); use the earlier messages only as context to interpret and attribute it correctly.',
    'AUTHORSHIP TEST — apply before storing any first-person statement: is the user telling you about THEMSELVES, or are they authoring/editing content (a document, story, speech, email, form, questionnaire, bio) or content for someone else? First-person statements INSIDE such content describe the subject of that content, NOT the user — return ignore for them. Only store a personal fact when the user is genuinely speaking about their own life or preferences. Example: while drafting a school speech for their 5-year-old ("My school is Keystone, I like cars, my favorite place is Fun Zone"), those are the child\'s speech lines, NOT facts about the user.',
    ...
    '  add:        {"op":"add","kind":"fact|preference|instruction|work_style|avoidance|procedure","text":"<concise statement>","entities":["<optional>"],"confidence":0.0-1.0,"salience":0.0-1.0,"sourceMessageIds":["<id of the source message>"],"reason":"<why>"}',
