// Typed client for the Azure OpenAI Responses API (/openai/v1/responses) — the single entry point
// for agentic chat (tool calls, multi-step state, typed output items). Ported from the browser
// `ai/responses.ts`; the only change is that `baseUrl` + `key` are passed in (vault-resolved).
...
export function toInputMessages(
  ...
  return turns.map((t) => {
    // Prior assistant turns are model OUTPUT and must use `output_text`; user/system turns use
    // `input_text`. Tagging an assistant turn as `input_text` makes the API reject the request (400).
