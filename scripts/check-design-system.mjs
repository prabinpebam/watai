...
//   3. raw native <select> elements outside src/design/ui.tsx — use SelectMenu instead.
...
const RAW_SELECT = /<select\b/;
const SELECT_ALLOWED = new Set(['src/design/ui.tsx']);
...
function checkFile(p) {
  ...
  lines.forEach((line, i) => {
    ...
    if (isTsx) {
      ...
      if (!SELECT_ALLOWED.has(rel) && RAW_SELECT.test(line)) {
        violations.push(`${rel}:${ln}  raw <select> — use the design-system SelectMenu component`);
      }
    }
  });
}
...
console.log('Design-system check passed: token colors and design-system controls are enforced.');
